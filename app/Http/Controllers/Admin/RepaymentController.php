...
class RepaymentController extends Controller
{
    ...
    protected function calculateLateFeeAtDate($schedule, $loan, $paymentDate)
    {
        ...
        $effectiveWaivedAmount = min($latepaymentOriginal, $totalWaivedAmount);
        $effectivePaidLateFeeRows = min(max(0, $latepaymentOriginal - $effectiveWaivedAmount), $totalPaidLateFeeRows);
        $latepayment = max(0, $latepaymentOriginal - $effectiveWaivedAmount - $effectivePaidLateFeeRows);
    ...
    {
        $canViewCompanyRepayments = $this->canViewCompanyRepayments($request);
        ...
        if ($canViewCompanyRepayments) {
            $this->loanAccessService->scopeRepaymentTableByLoanAccess($query, 'l', $request->user());
        } else {
            $query->where('r.added_by', $request->user()->id);
        }
    ...
    protected function getConfirmedSchedulePayments(array $scheduleIds, string $table = 'repayments'): array
    {
        ...
        if (Schema::hasColumn($table, 'status') && Schema::hasColumn($table, 'payment_status')) {
    ...
    protected function canViewCompanyRepayments(Request $request): bool
    {
        $user = $request->user();

        return (bool) ($user?->isSuperAdmin() || $user?->isAdministrator());
    }
    ...
    protected function calculateLateFeeWithWaiverAmount($schedule, $loan, float $waivedAmount): array
    {
        ...
        $effectiveWaivedAmount = min($gross, $waivedAmount);

        return [
            'gross' => $gross,
            'net' => max(0, $gross - $effectiveWaivedAmount),
            'waived' => $effectiveWaivedAmount,
    ...
    protected function attachLoanCollateralStatus($loans): void
    {
        ...
        $personalIds = $loans
            ...
            ->all();
        $memberIds = $loans
            ->filter(fn ($loan) => ($loan->loan_type ?? null) === 'personal' && !empty($loan->member_id))
            ->pluck('member_id')
            ->values()
            ->all();
        ...
        $cashSecurities = CashSecurity::query()
            ...
            ->where(function ($query) use ($personalIds, $memberIds) {
                $query->whereIn('loan_id', $personalIds);

                if (!empty($memberIds)) {
                    $query->orWhere(function ($memberQuery) use ($memberIds) {
                        $memberQuery->whereNull('loan_id')
                            ->whereIn('member_id', $memberIds);
                    });
                }
            })
            ->get();
        $loanCashSecurities = $cashSecurities->whereNotNull('loan_id')->groupBy('loan_id');
        $memberCashSecurities = $cashSecurities->whereNull('loan_id')->groupBy('member_id');
        ...
        foreach ($loans as $loan) {
            $linkedCashSecurities = ($loan->loan_type ?? null) === 'personal'
                ? $loanCashSecurities
                    ->get($loan->id, collect())
                    ->merge($memberCashSecurities->get($loan->member_id, collect()))
                : collect();
            ...
            ? CashSecurity::query()
                ...
                ->where(function ($query) use ($loan) {
                    ...
                    if (!empty($loan->member_id)) {
                        $query->orWhere(function ($memberQuery) use ($loan) {
                            $memberQuery->whereNull('loan_id')
    ...
    protected function activeGroupLoansOptimized(Request $request)
    {
        ...
        if ($this->activeLoanPageNeedsAssignment($request)) {
            $relations[] = 'assignedTo:id,name,branch_id,designation';
        }
        ...
        if ($this->activeLoanPageNeedsFollowUps($request)) {
            $this->attachLoanFollowUps($pageLoans);
        }

        if ($this->activeLoanPageNeedsCollateral($request)) {
    ...
    protected function activeLoanPageNeedsAssignment(Request $request): bool
    {
        $page = $request->get('active_page', 'schedule');

        return in_array($page, ['collections', 'risk', 'security', 'operations'], true);
    }

    protected function activeLoanPageNeedsFollowUps(Request $request): bool
    {
        $page = $request->get('active_page', 'schedule');
        $status = $request->get('status');

        return in_array($page, ['collections', 'risk', 'operations'], true)
            || in_array($status, ['risk_followup', 'missing_followup'], true);
    }

    protected function activeLoanPageNeedsCollateral(Request $request): bool
    {
        $page = $request->get('active_page', 'schedule');
        $status = $request->get('status');

        return in_array($page, ['security', 'operations'], true)
            || $status === 'missing_collateral';
    }
    ...
    private function activeLoanViewOptions(Request $request): array
    {
        ...
        $assignableUsers = $page === 'operations' && $canReassignLoans
            ? $this->loanAccessService->assignableLoanUsers()
            : collect();
        ...
        return [
            ...
            'canReassignLoans' => $canReassignLoans,
    ...
    protected function getWaivedLateFeesBySchedule(array $scheduleIds): array
    {
        ...
        return DB::table('late_fees')
            ->whereIn('schedule_id', $scheduleIds)
            ->whereIn('status', [LateFee::STATUS_PAID, LateFee::STATUS_WAIVED])
            ->groupBy('schedule_id')
            ->select('schedule_id', DB::raw('SUM(amount) as total_settled'))
            ->pluck('total_settled', 'schedule_id')
    ...
    protected function getFastActivePersonalLoanStats(Request $request): array
    {
        ...
        try {
            ...
            $needsFollowUpStats = $this->activeLoanPageNeedsFollowUps($request);
            $needsCollateralStats = $this->activeLoanPageNeedsCollateral($request);
            ...
            if (!empty($loanIds)) {
                ...
                $waivedSubquery = DB::table('late_fees')
                    ->whereIn('status', [LateFee::STATUS_PAID, LateFee::STATUS_WAIVED])
                    ->groupBy('schedule_id')
                    ->select('schedule_id', DB::raw('SUM(amount) as settled_late_fees'));
                ...
                $aggregate = DB::table('loan_schedules as s')
                    ...
                    ->selectRaw("
                        ...
                        SUM(GREATEST(0, {$grossLateFeeExpression} - COALESCE(w.settled_late_fees, 0) - {$lateFeePaidExpression})) as outstanding_late_fees
                    ")
                    ->first();
                ...
                if ($needsCollateralStats && Schema::hasTable('cash_securities')) {
    ...
    protected function getFastActiveGroupLoanStats(Request $request): array
    {
        ...
        try {
            ...
            if (!empty($loanIds)) {
                ...
                if (Schema::hasColumn('group_repayments', 'status') && Schema::hasColumn('group_repayments', 'payment_status')) {
                    $paidSubquery->whereNotIn('status', [-1, 2])
                        ->where(function ($q) {
                            $q->where('status', 1)->orWhere('payment_status', 'Completed');
                        });
                } elseif (Schema::hasColumn('group_repayments', 'status')) {
                    $paidSubquery->where('status', 1);
                }

                $paidSubquery->groupBy('schedule_id')
                    ->select('schedule_id', DB::raw('SUM(amount) as paid'));
    ...
    {
        $requestedLoanType = request('type');
        abort_unless(in_array($requestedLoanType, [null, 'personal', 'group'], true), 422, 'Unsupported loan type.');

        $personalLoanLookup = function () use ($loanId) {
            return PersonalLoan::with(['member', 'branch', 'product', 'schedules.repayments', 'repayments', 'disbursements' => function($query) {
                    $query->where('status', 2)->orderBy('created_at', 'desc');
                }])
        ...
        $groupLoanLookup = function () use ($loanId) {
            return GroupLoan::with(['group', 'branch', 'product', 'schedules.repayments', 'repayments', 'disbursements' => function($query) {
                        $query->where('status', 2)->orderBy('created_at', 'desc');
                    }])
                       ->whereIn('status', [2, 3, 5]) // Disbursed, Completed, or Restructured
                       ->find($loanId);
        ...
        if ($requestedLoanType === 'group') {
            $loan = $groupLoanLookup();
            $loanType = 'group';
        } else {
            $loan = $personalLoanLookup();
            $loanType = 'personal';
        }

        if (!$loan && $requestedLoanType === null) {
            $loan = $groupLoanLookup();
            $loanType = 'group';
        }
        ...
        $repaymentTable = $loanType === 'group' ? 'group_repayments' : 'repayments';
        $repaymentModelClass = $loanType === 'group' ? GroupRepayment::class : Repayment::class;
        $repaymentDateColumn = $loanType === 'group' ? 'created_at' : 'date_created';
        $canRecordSchedulePayments = $loanType === 'personal';
        ...
        $paidPerSchedule = $this->getConfirmedSchedulePayments($scheduleIds, $repaymentTable);
        ...
        $schedules = $loan->schedules->map(function($schedule, $index) use ($loan, &$principal, &$globalprincipal, $paidPerSchedule, $lateFeesPaidPerSchedule, $pendingPerSchedule, $repaymentTable, $repaymentDateColumn, $loanType) {
            ...
            if ($quickBalanceCheck <= 0.01) {
                ...
                if ($schedule->date_cleared) {
                    ...
                    $lastPaymentQuery = DB::table($repaymentTable)
                        ->where('schedule_id', $schedule->id)
                        ->where('amount', '>', 0);

                    if ($loanType === 'personal') {
                        $lastPaymentQuery->where(function($query) {
                            $query->where('status', 1)
                                  ->orWhere('payment_status', 'Completed');
                        });
                    }

                    $lastPayment = $lastPaymentQuery
                        ->orderBy('id', 'desc')
                        ->first();
                    
                    $now = $lastPayment ? strtotime($lastPayment->{$repaymentDateColumn}) : time();
                }
            } else {
            ...
            if ($totalBalance <= 0.01) {
                ...
                if ($schedule->date_cleared) {
                    ...
                    $paymentsQuery = DB::table($repaymentTable)
                        ->where('schedule_id', $schedule->id)
                        ->where('amount', '>', 0);

                    if ($loanType === 'personal') {
                        $paymentsQuery->where('status', 1);
                    }

                    $payments = $paymentsQuery->orderBy('id', 'asc')->get();
                    
                    if (count($payments) > 0) {
                        $lastPayment = $payments->last();
                        $paymentDate = strtotime($lastPayment->{$repaymentDateColumn});
                    } else {
                        $paymentDate = time();
                    }
                }
                ...
                // Check for waiver/payment rows recorded against this late fee.
            ...
            $totalPaidLateFeeRows = $lateFeeData['paid'] ?? 0;
            ...
            $schedule->penalty_paid_recorded = $totalPaidLateFeeRows; // Paid via late_fees records/carry-over
        ...
        $lateFeesOutstanding = $schedules->sum(function ($schedule) {
            return max(0, (float) ($schedule->penalty ?? 0) - (float) ($schedule->penalty_paid ?? 0));
        });
        ...
        $totalPaidQuery = DB::table($repaymentTable)
            ->where('loan_id', $loan->id)
            ->where('amount', '>', 0);

        if ($loanType === 'personal') {
            $totalPaidQuery->where('status', 1);
        }
        ...
        return view('admin.loans.repayments.schedules', compact(
            ...
            'totalLateFees', 'lateFeesWaived', 'lateFeesPaid', 'lateFeesOutstanding',
            'repaymentModelClass', 'canRecordSchedulePayments'
        ));
    }
    ...
    public function quickRepayment(Request $request)
    {
        ...
        try {
            ...
            if (!$schedule || (int) $schedule->loan_id !== (int) $loan->id) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Selected schedule does not belong to this loan.'
                ], 422);
            }

            $sequenceCheck = $this->checkEarlierSchedulesPaid($schedule->id, $loan->id);
            ...
            $requiredAmount = round((float) $components['outstanding']);
            ...
            if (abs((float) $request->amount - $requiredAmount) > 1) {
                DB::rollBack();
                return response()->json([
                    'success' => false,
                    'message' => 'Payment must equal the exact selected schedule balance: UGX ' . number_format($requiredAmount, 0),
            ...
            if ($paymentTypeCode == 2) {
                ...
                if ($mobileMoneyResult['success']) {
                    ...
                    $repaymentData['payment_status'] = 'Pending';
                    $repaymentData['payment_phone'] = $phone;
            ...
            if ($repaymentData['status'] == 1) {
                $this->repaymentService->syncScheduleSettlement(
                    LoanSchedule::findOrFail($repayment->schedule_id),
                    $loan
                );
            ...
            if ($paymentTypeCode == 2) {
                ...
                $jsonResponse['transaction_reference'] = $repayment->txn_id ?: $repayment->transaction_reference;
            }
            if (!$glPosted) {
    ...
    public function storeRepayment(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('admin.loans.repayments.schedules', [
                'id' => $request->loan_id,
                'type' => $loanType,
            ])
    ...
    public function index(Request $request)
    {
        ...
        if (in_array($loanType, ['school', 'student', 'staff'])) {
            ...
            $branches = $this->loanAccessService
                ->branchesForActiveLoanOperations(Branch::active(), $request->user())
                ->get() ?? collect();
            $kpiMonth = $request->get('kpi_month');
            $kpiPeriodLabel = 'No supported repayment records';
            
            return view('admin.repayments.index', compact('repayments', 'branches', 'totals', 'loanType', 'kpiMonth', 'kpiPeriodLabel', 'canViewCompanyRepayments'))
        ...
        if ($canViewCompanyRepayments) {
            $this->loanAccessService->scopeRepaymentQueryByLoanAccess($query, 'loan', $request->user());
        } else {
            $query->where('added_by', $request->user()->id);
        }

        if (!$canViewCompanyRepayments || $repaymentView === 'completed') {
            $query->where(function ($q) {
                $q->where('status', 1)
                    ->orWhere('payment_status', 'Completed');
            });
        } elseif ($repaymentView === 'pending') {
        ...
        if ($canViewCompanyRepayments) {
            $this->loanAccessService->scopeRepaymentQueryByLoanAccess($feesQuery, 'loan', $request->user());
        } else {
            $feesQuery->where('added_by', $request->user()->id);
        }
        ...
        return view('admin.repayments.index', compact(
            ...
            'kpiPeriodLabel', 'repaymentView', 'pageTitle', 'canViewCompanyRepayments'
        ));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($isMobileMoney) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'Please use the loan schedules page for Mobile Money repayments so the gateway request, callback, and schedule update are linked correctly.');
        }
        ...
        if (!$isConfirmed) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'Cash and bank repayments must be confirmed immediately by the Super Administrator.');
        }

        try {
            DB::beginTransaction();

            $loan = Loan::with('product')->find($validated['loan_id']);
            ...
            $scheduleQuery = LoanSchedule::where('loan_id', $loan->id)
                ->where('status', '!=', 1)
                ->orderBy('payment_date')
                ->orderBy('id');
            ...
            if (!$schedule) {
                ...
                return redirect()->back()
                    ->withInput()
                    ->with('error', 'No unpaid schedule was found for this loan.');
            }

            $paymentDate = !empty($validated['date']) ? Carbon::parse($validated['date']) : now();
            $components = $this->repaymentService->getScheduleOutstandingComponents($schedule, $loan, $paymentDate);
            $requiredAmount = round((float) $components['outstanding']);
            ...
            if ($requiredAmount <= 1) {
                ...
                return redirect()->back()
                    ->withInput()
                    ->with('error', 'This installment has already been fully paid.');
            }

            if (abs((float) $validated['amount'] - $requiredAmount) > 1) {
                DB::rollBack();
                return redirect()->back()
                    ->withInput()
                    ->with('error', 'Payment must equal the exact selected schedule balance: UGX ' . number_format($requiredAmount, 0));
            }

            $reference = $validated['txn_id'] ?: sprintf(
                '%s-%s-%04d',
                (int) $validated['type'] === 1 ? 'CH' : 'BT',
                now()->format('YmdHisv'),
                mt_rand(1000, 9999)
            );

            $paymentMethodName = (int) $validated['type'] === 1 ? 'Cash' : 'Bank Transfer';
            ...
            $repaymentData = [
                ...
                'pay_status' => 'SUCCESS',
                'pay_message' => $paymentMethodName . ' payment confirmed by ' . auth()->user()->name,
            ];

            $repayment = Repayment::create($repaymentData);

            $this->repaymentService->syncScheduleSettlement($schedule->fresh(), $loan, $paymentDate);
    ...
    public function getLoanDetails(Loan $loan)
    {
        ...
        $components = $nextDue
            ? $this->repaymentService->getScheduleOutstandingComponents($nextDue, $loan)
            : null;
        ...
        return response()->json([
            ...
            'loan' => [
                ...
                'next_schedule_id' => $nextDue ? $nextDue->id : null,
                'next_due_date' => $nextDue ? $nextDue->payment_date : null,
                'next_due_amount' => $components ? round((float) $components['outstanding']) : 0,
                'principal_due' => $components ? (float) $components['principal'] : 0,
                'interest_due' => $components ? (float) $components['interest'] : 0,
                'penalty' => $components ? (float) $components['late_fee_net'] : 0,
    ...
    public function waiveLateFees(Request $request)
    {
        try {
            ...
            foreach ($lateFees as $lateFee) {
                ...
                if (isset($lateFee['late_fee_id'])) {
                    ...
                    if ($updated) {
                        ...
                        $affectedScheduleIds[] = (int) $lateFeeRecord->schedule_id;
                    } else {
                        $failedIds[] = $lateFeeId;
                    }
                    
                } else if (isset($lateFee['schedule_id'])) {
                    // Calculated penalty from schedule - calculate fresh server-side,
                    // then create a waiver only for the currently outstanding net fee.
                    $scheduleId = $lateFee['schedule_id'];
                    
                    // Get schedule details
                    $schedule = $loan instanceof GroupLoan
                        ? GroupLoanSchedule::where('id', $scheduleId)->lockForUpdate()->first()
                        : LoanSchedule::where('id', $scheduleId)->lockForUpdate()->first();
                    ...
                    $lateFeeData = $this->repaymentService->calculateLateFee($schedule, $loan);
                    $amount = round((float) ($lateFeeData['net'] ?? 0), 2);
                    ...
                    $dueTimestamp = $this->parsePaymentDate($schedule->payment_date);
                    $scheduleDueDate = $dueTimestamp ? date('Y-m-d', $dueTimestamp) : $schedule->payment_date;
                    ...
                    $inserted = DB::table('late_fees')->insert([
                        ...
                        'member_id' => $memberId,
                        'amount' => $amount,
                        'days_overdue' => (int) ($lateFeeData['days_overdue'] ?? 0),
                        'periods_overdue' => (int) ($lateFeeData['periods_overdue'] ?? 0),
                        'period_type' => optional($loan->product)->period_type,
                        'schedule_due_date' => $scheduleDueDate,
                        'calculated_date' => now(),
                        'calculation_details' => json_encode([
                            'gross' => (float) ($lateFeeData['gross'] ?? 0),
                            'already_waived' => (float) ($lateFeeData['waived'] ?? 0),
                            'already_paid' => (float) ($lateFeeData['paid'] ?? 0),
                        ]),
                    ...
                    if ($inserted) {
                        $waivedCount++;
                        $totalWaived += $amount;
                        $affectedScheduleIds[] = (int) $scheduleId;
                    } else {
                        $failedIds[] = $scheduleId;
                    }
                }
            ...
            $settledSchedules = 0;
            if ($waivedCount > 0 && $loan instanceof PersonalLoan) {
                $loan->loadMissing('product');
                $affectedSchedules = LoanSchedule::whereIn('id', array_unique($affectedScheduleIds))->get();

                foreach ($affectedSchedules as $affectedSchedule) {
                    if ($this->repaymentService->syncScheduleSettlement($affectedSchedule, $loan)) {
                        $settledSchedules++;
                    }
                }

                $this->repaymentService->checkAndCloseLoanIfComplete((int) $loan->id);
            }
            ...
            Log::info("Late fees waived", [
                ...
                'settled_schedules' => $settledSchedules,
                'reason' => $waiverReason,
        ...
        try {
            ...
            if ($validated['target_action'] === 'next_schedule') {
                ...
                foreach ($lateFees as $lateFee) {
                    ...
                    if ($paymentAmount >= $lateFee->amount) {
                        DB::table('late_fees')->where('id', $lateFee->id)->update([
                            'status' => LateFee::STATUS_PAID,
                            'paid_at' => now(),
                            'payment_reference' => 'CARRY-' . $validated['schedule_id'],
                            'updated_at' => now(),
                        ]);
                    }
                ...
                $this->repaymentService->syncScheduleSettlement(
                    LoanSchedule::findOrFail($validated['schedule_id'])
                );
            ...
            if ($targetSchedule) {
                ...
                $carryReference = 'CARRY-' . $validated['schedule_id'] . '-TO-' . $targetSchedule->id . '-' . now()->format('YmdHis') . '-' . mt_rand(1000, 9999);
                
                // Create audit log
                Repayment::create([
                    'type' => 1,
                    'details' => ($validated['carry_note'] ?? 'Manual carry-over') . ' (From Schedule ' . $validated['schedule_id'] . ')',
                    'loan_id' => $validated['loan_id'],
                    ...
                    'payment_status' => 'Completed',
                    'pay_status' => 'SUCCESS',
                    'pay_message' => 'Manual carry-over from schedule ' . $validated['schedule_id'],
                    'txn_id' => $carryReference,
                    'transaction_reference' => $carryReference,
                    'platform' => 'CarryOver',
                    'date_created' => now(),
                    'added_by' => auth()->id()
                ]);
                ...
                $this->repaymentService->syncScheduleSettlement(
                    LoanSchedule::findOrFail($targetSchedule->id)
                );
