...
class UpdatedSystemAccountsSeeder extends Seeder
{
    /**
     * Run the database seeds.
     * 
     * Updated Chart of Accounts with control flags for double-entry system
     */
    public function run(): void
    {
        $this->command->info('🔄 Updating Chart of Accounts with control flags...');
        ...
        $this->command->info('✅ Chart of Accounts updated successfully with ' . count($accounts) . ' accounts.');
        $this->command->info('🔒 System-controlled accounts (allow_manual_posting = FALSE):');
        $this->command->info('   - All Cash/Bank accounts (10000-10041)');
        $this->command->info('   - All Clearing accounts (10100-10130)');
        $this->command->info('   - All Loan Receivables (11000-11040)');
        $this->command->info('   - All Loan Provisions (11100-11140)');
        $this->command->info('   - Cash Collateral (21025)');
        $this->command->info('   - Insurance Payables (21030, 21040)');
        $this->command->info('   - Client Onboarding Fees (42040)');
        $this->command->info('   - Loan Admin Fees (42050)');
        $this->command->info('   - Loan Loss Expenses (56000, 56100)');
    }
}
